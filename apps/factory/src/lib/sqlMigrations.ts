...
// Keep this constant stable. Advisory locks are per-database, so a single key is enough.
...
export async function runSqlMigrations(databaseUrl: string, migrationsDir: string) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      console.log(`[factory] applying tenant migration: ${file}`);
