...
- [x] Fix any failing tests on main/PRs so AMD CI is green by default (start with alerts-core if failing). (Ensured alerts-core rejects invalid timestamps so outputs stay ISO-compliant.)
